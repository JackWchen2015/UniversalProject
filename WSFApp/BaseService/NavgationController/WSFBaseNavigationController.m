//
//  WSFBaseNavigationViewController.m
//  WSFApp
//
...
#import "WSFTransition.h"
#import "WSFTransitionProtocol.h"
#import "WSFBaseViewController.h"
@interface WSFBaseNavigationController()<UINavigationControllerDelegate,UIGestureRecognizerDelegate>
...
@implementation WSFBaseNavigationController
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    if (self.viewControllers.count > 0) {
        if ([viewController conformsToProtocol:@protocol(WSFTransitionProtocol)] && [self isNeedTransition:viewController]) {
...
-(void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    if ([viewController isKindOfClass:[WSFBaseViewController class]]) {
        WSFBaseViewController * vc = (WSFBaseViewController *)viewController;
...
-(id<UIViewControllerAnimatedTransitioning>)navigationController:(UINavigationController *)navigationController animationControllerForOperation:(UINavigationControllerOperation)operation fromViewController:(UIViewController *)fromVC toViewController:(UIViewController *)toVC
{
    ...
    if ([fromVC conformsToProtocol:@protocol(WSFTransitionProtocol)] && [toVC conformsToProtocol:@protocol(WSFTransitionProtocol)]) {
        
        BOOL pinterestNedd = [self isNeedTransition:fromVC:toVC];
        WSFTransition *transion = [WSFTransition new];
        ...
        return transion;
    }else if([toVC conformsToProtocol:@protocol(WSFTransitionProtocol)]){
...
-(BOOL)isNeedTransition:(UIViewController<WSFTransitionProtocol> *)fromVC :(UIViewController<WSFTransitionProtocol> *)toVC
{
    BOOL a = NO;
    BOOL b = NO;
...
//判断fromVC和toVC是否需要实现pinterest效果
-(BOOL)isNeedTransition:(UIViewController<WSFTransitionProtocol> *)toVC
{
    BOOL b = NO;
    if ([toVC respondsToSelector:@selector(isNeedTransition)] && [toVC isNeedTransition]) {
        b = YES;
