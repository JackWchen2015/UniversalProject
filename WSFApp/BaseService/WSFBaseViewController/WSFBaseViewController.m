...
#import "WSFNoDataView.h"
#import <MJRefresh/MJRefresh.h>
@interface WSFBaseViewController ()
{
    BOOL  isRefreshing;
}
@property(nonatomic)WSFBaseTipViewController* baseTipController;
@property(nonatomic)WSFNoDataView*  noDataView;
@end
...
#pragma mark --  TipViewController
-(WSFBaseTipViewController *)baseTipController
{
    if(!_baseTipController)
    {
        _baseTipController=[WSFBaseTipViewController new];
    }
    return _baseTipController;
}
...
- (void)showRefreshingViewWithTip:(NSString *)tip {
    isRefreshing = YES;
    [self.baseTipController startAnimateWithTip:tip];
    [self addChildViewController:self.baseTipController];
    self.baseTipController.view.frame = self.view.bounds;
    [self.view addSubview:self.baseTipController.view];
    [self.baseTipController didMoveToParentViewController:self];
}

- (void)hiddenRefreshingView {
    if (isRefreshing) {
        [UIView animateWithDuration:0.25f animations:^{
            self.baseTipController.view.alpha = 0;
        } completion:^(BOOL finished) {
            isRefreshing = NO;
            [self.baseTipController stopAnimateWithTip:nil];
            [self.baseTipController willMoveToParentViewController:nil];
            [self.baseTipController.view removeFromSuperview];
            [self.baseTipController removeFromParentViewController];
            self.baseTipController = nil;
        }];
    }
}
...
- (void)showErrorViewWithTip:(NSString *)tip {
    isRefreshing = NO;
    [self.baseTipController stopAnimateWithTip:tip];
}
 ...
 *  是否显示返回按钮
 */
- (void) setIsShowLeftBack:(BOOL)isShowLiftBack
{
    _isShowLeftBack = isShowLiftBack;
...
@implementation WSFBaseViewController (WSFSubclassViewController)
