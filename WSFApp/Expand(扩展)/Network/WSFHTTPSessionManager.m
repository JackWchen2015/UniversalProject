...
static WSFHTTPSessionManager *wsfSessionManager= nil;
@implementation WSFHTTPSessionManager

#pragma mark - 实现声明单例方法 GCD
+(instancetype)shareManager{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        wsfSessionManager = [[WSFHTTPSessionManager alloc]init];
    });
    return wsfSessionManager;
}
+ (instancetype)allocWithZone:(struct _NSZone *)zone {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        wsfSessionManager = [super allocWithZone:zone];
    });
    return wsfSessionManager;
}
+ (instancetype)init {
...
#pragma mark - GET请求
+ (WSFURLSessionTask *)GET:(NSString *)URL parameters:(NSDictionary *)parameters progress:(HttpProgress)progress success:(HttpRequestSuccess)success failure:(HttpRequestFailed)failure
{
...
#pragma mark - POST请求
+ (WSFURLSessionTask *)POST:(NSString *)URL parameters:(NSDictionary *)parameters progress:(HttpProgress)progress success:(HttpRequestSuccess)success failure:(HttpRequestFailed)failure
{
 ...
 *  @param progress 进度
 */
+ (WSFURLSessionTask *)ba_requestWithType:(XDHttpRequestType)type UrlString:(NSString *)urlString Parameters:(NSDictionary *)parameters progress:(HttpProgress)progress SuccessBlock:(HttpRequestSuccess)successBlock FailureBlock:(HttpRequestFailed)failureBlock
{
    if (urlString == nil)
    {
        return nil;
    ...
    WSFURLSessionTask *sessionTask = nil;
...
#pragma mark - 上传图片文件
+ (WSFURLSessionTask *)uploadWithURL:(NSString *)URL parameters:(NSDictionary *)parameters images:(NSArray<UIImage *> *)images name:(NSString *)name fileName:(NSString *)fileName mimeType:(NSString *)mimeType progress:(HttpProgress)progress success:(HttpRequestSuccess)success failure:(HttpRequestFailed)failure
{
    [UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
...
#pragma mark - 下载文件
+ (WSFURLSessionTask *)downloadWithURL:(NSString *)URL fileDir:(NSString *)fileDir progress:(HttpProgress)progress success:(void(^)(NSString *))success failure:(HttpRequestFailed)failure
{
    [UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
...
+ (WSFURLSessionTask *)requestWithURLString:(NSString *)URLString
